"""
This test file is used to test NexusDownloadFlow's v1.0.0 algorithm.
...
def init_templates() -> list[MatLike]:
    return [cv2.cvtColor(template, cv2.COLOR_BGR2GRAY) for template in load_templates()]


def test_algorithm() -> None:
    logging_test("ndf-1.0.0-grayscale-template-matching.")
    logging_test("Comparaison method: TM_SQDIFF.")
    try:
